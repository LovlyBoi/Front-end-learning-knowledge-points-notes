...
JS 引擎为了能执行代码，会创建一个执行上下文栈，在执行每一个上下文时，会将它推入执行上下文栈，该栈的栈底是全局上下文，每个上下文会在它所有代码执行完毕后销毁出栈，（函数每次执行都会产生一个函数上下文）
...
拿到 html 和 css，解析 html 生成 DOM 树，解析 CSS 生成 CSS 规则树，再结合二者形成渲染树（render tree）。渲染树被布局引擎修改后，绘制并展示在屏幕上。
...
引擎拿到代码 -> 词法分析生成 tokens，并生成全局对象GO -> 语法分析生成 AST （解析和预解析）-> 生成全局执行上下文，其中变量对象 VA 指向 GO（生成时先检索函数声明，完成函数引用，再检索变量并初始化，变量和函数都挂载在 GO 上）-> 全局上下文入栈执行 -> ......
...
在根目录下（和 src 平级）创建 mock 文件夹，在该文件夹下创建 JS 文件
