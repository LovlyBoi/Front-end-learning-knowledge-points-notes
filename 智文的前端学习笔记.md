...
[flex 布局的应用](https://www.bilibili.com/video/BV1xz4y1D7pA?spm_id_from=333.999.0.0)
...
- 作用域链：头部为当前VO，后面是父级作用域链（`[variable object + [[scope]] ]`）
...
function debounce(fn, delay){
  let timer
  return function(...args){
    // this指向问题需要考虑，定时器中的this指向window了，需要重新绑定
    let that = this
    clearTimeout(timer)
    timer = setTimeout(() => {
      fn.apply(that, args)
...
异步任务分为两种：**微任务**（microtask）和**宏任务**（macrotask）。不同的任务会被分到不同的任务队列。

以定时器举例，在调用 setTimeout 时，我们只是向浏览器发送了一条指令，在规定时间后执行指定的回调函数，这里这个回调函数是异步的，浏览器会让别的线程来计时，等到时间到了会将这个回调函数推入一个事件队列（宏任务队列）中。这个事件队列是浏览器维护的，当 JS 线程处理完所有同步代码之后，来这个队列里取出回调函数执行。重复的这个循环叫做事件循环。

规范定义在执行宏任务之前，必须**保证所有微任务被清空**了。

在 JS 线程执行完所有代码后，先查看微任务队列，执行其中的**所有**代码，然后再去查看宏任务队列，取出**一个**宏任务放入 JS 执行线程中执行。

微任务主要包括：

- Promise.then 回调函数
- await 后面的代码（即使 await 了一个非 Promise）
- MutationObserver（监视 DOM 变化）
- Object.observe（监视对象变化）
- process.nextTick（node，在下一次事件循环 tick 时执行回调）
- queueMicrotask()（这个函数可以将回调函数加入微任务队列）

宏任务主要包括：

- setTimeoout / setInterval 回调函数
- AJAX 回调函数
- DOM 回调函数
- UI Rendering
...
Node 中的事件循环由 libuv 这个库实现。libuv 中主要维护了一个 EventLoop 和 worker threads（线程池）。

EventLoop 负责调用系统的一些其他操作：文件的IO、Network、child-processes 等。这些操作执行完，可以将结果回调放入对应的事件队列中，JS 线程再来执行。

Node 的事件循环更复杂一些。一次完整的循环（Tick）分成很多个阶段：

- 定时器（Timers）：执行 setTimeout / setInterval 的回调函数。
- 待定回调（Pending Callback）：对某些系统操作（如 TCP 错误类型）执行回调。
- idle，prepare：仅系统内部使用。
- 轮询（Poll）：检索新的 I/O 事件，执行相关 I/O 回调。
- 检测（Check）：setImmediate 回调在此时执行。
- 关闭的回调函数：一些关闭的回调函数，如：socked.on('close', ...)。

这些阶段都有自己的任务队列，而且都是宏任务队列。

微任务的优先级同样很高，会保证执行宏任务之前将微任务全部执行完。微任务和宏任务的划分基本和浏览器一样，但是 Node 会多出来一些 API。

微任务队列有：

- next tick queue：process.nextTick，这个队列比 other queue 优先执行。
...
- timer queue：定时器回调
- poll queue：I/O 事件
- check queue：setImmediate 回调（所以定时器比 setImmediate 先执行）
...
迭代器是确使用户可以在容器对象上遍历的对象，使用该接口无需关系对象内部实现细节。其行为类似数据库中的 cursor。

在 JS 中，迭代器是一个具体的对象，这个对象需要符合迭代器协议（Iterator Protocal）：需要到包含一个特定的 next 方法。next 方法需要符合以下要求：

- 是一个函数，接收一个或零个参数，返回一个对象，该对象需要包含以下两个属性：
  - done：一个布尔值，表示是否迭代完成。
...
可迭代对象是一个实现了 `@@iterable` 方法的对象，我们可以使用 `Symbol.iterator` 访问该属性。

该属性是一个函数，返回一个迭代器。

可迭代对象可以使用 for...of 来进行遍历。
...
const obj = {
  ...
  [Symbol.iterator]() {
    return {
      index: 0,
      next() {
        if (this.index < obj.nums.length) {
          return { value: obj.nums[this.index++] }
        }
        else {
          return { done: true }
        }
...
class ClassRoom {
  ...
  [Symbol.iterator]() {
    ...
    return {
      next() {
        if (i < that.students.length) {
          return { value: that.students[i++] }
        }
        else {
          return { done: true }
        }
...
const room1 = new ClassRoom('101', ['Tom', 'Siven', 'Steve'])
for(let student of room1){
  console.log(student)
}
```
...
生成器是 ES6 中新增的一种函数控制、使用的方案，他可以让我们更灵活的控制函数什么时候继续执行、暂停执行等。普通函数的执行是不可以暂停的。

生成器函数也是一个函数，但有一些区别：

- 首先，生成器函数需要在 function 后面加一个 * 号。
- 其次，生成器函数可以通过 yield 关键字来控制函数的执行流程。
- 最后，生成器函数的返回值是一个 Generator。
...
生成器函数在被调用时不会执行，而是返回一个生成器对象。

函数被 yield 关键字划分为多个段，每次调用生成器的 next 方法都会执行一段。每一段可以有自己的返回值，跟在 yield 后面即可。
...
return 也可以返回一个值，但是 return 之后的代码不会再执行。

如果希望每一段执行时传入参数，参数可以在 next 中传入，在上一段 yield 的返回值中拿到。
...
console.log(g.next())
console.log(g.next(10)) // 10 2
```

调用生成器的 next 方法，可以让函数接着执行下一段；调用 return 方法，可以让函数直接返回，停止执行，也可以传入参数。调用后下一段代码不会执行，但会返回上一段的返回值；调用 throw 方法，可以抛出异常。

**使用生成器来替代迭代器**

因为生成器也是返回一个迭代器，我们可以来代替迭代器
...
ES6 引入了 Symbol 类型，解决了命名冲突的问题。每一个 Symbol 值都是独一无二的，可以保证不会和其他属性产生命名冲突。
...
3. ###### Promise 使用
   ...
   1. 创建 Promise 实例
      ...
      - 是一个正常的值，则 `resolve` 调用的时候会将这个值传递给 then 的回调。
      - 是另一个 Promise，这时，当前这个 Promise 的解决状态就取决于那个 Promise 了，相当于转移了状态。
      - 是一个实现了 thenable 接口的对象，这时会执行 then 方法，状态取决于那个 then 方法的返回值。
   ...
   2. then 方法
      ...
      ​	**参数**：`then` 方法接收最多两个参数，分别是 `onResolved(value)` 和 `onReject(reason)`，通过传入的 `value` 参数和 `reason` 参数我们可以在这两个函数内分别进行成功处理和失败处理。由于期约只能落定一次，所以这两个函数只有一个能执行。

      ​	**返回值**：`then` 方法的返回值是一个新的 `Promise` 对象，这样我们就可以将 .then 进行==链式调用==。这个新的 `Promise` 对象会通过 ==Promise.resolve()== 来包装**成功处理程序**或**失败处理程序**的**返回值**（成功和失败都会返回一个成功的，确实有点奇怪，但是仔细想想又没有错）。如果没有提供处理程序，将会使用 `Promise.resolve()` 来包装上一个期约解决后的值。如果处理程序中没有返回值，则会包装默认的 `undefined`。在处理程序中抛出异常则会返回一个 `rejected` 的期约。

      ​	总结一下这个返回值的问题：

      1. 如果不抛出异常，即用 Promise.resolve 包裹返回值，进入 Promise.resolve 的处理流程：
         1. 如果返回一个普通值（无论是在成功回调还是失败回调），那么使用 Promise.resolve 包裹这个普通值返回，如果没有返回值，那么返回 Promise.resolve(undefined)。
         2. 如果返回一个 Promise，那么直接返回的就是这个 Promise。相当于对 Promise.resolve 传入了另一个 Promise，即将 Promise 转移了状态。
         3. 如果返回的是一个实现了 thenable 接口的对象，那么执行他的 then 方法，返回值取决于这个 then 方法。
      2. 如果回调处理程序抛出异常，那么使用 Promise.reject 来包裹异常返回。
      ...
      同一个 Promise 也可以被调用多次 then 方法。

      ```js
      const p = new Promise.resolve('123')
      ...
      当有多个 then 方法时，resolve 被执行时，所有的 then 方法的回调都会被执行。
...
async 表示异步，添加这个关键字的函数表示这是一个异步函数。

但是这个异步函数和正常函数还是有一些区别的：

1. 返回值

   添加 async 关键字的异步函数的返回值一定是一个 Promise。其返回值转换为 Promise 的逻辑与 then 的回调相同。

2. 异常处理

   普通函数抛出错误时，程序会直接崩溃（如果没有 try catch）。但是异步函数中抛出的异常会被 Promise.reject 包裹返回，可以在函数返回的 Promise 的 catch 方法中处理。

当然，async 最特殊的地方还是在于：函数中可以使用 await 关键字。

通常使用 await 关键字时后面可以跟着一个表达式，这个表达式会返回一个 Promise。那么 await 会等到这个 Promise 变为 fulfilled 时，继续执行后面的代码。如果表达式返回一个普通值，那么会直接返回这个值。

如果 await 后面的表达式变为了 rejected 状态，代码会报错。所以需要在 async 函数调用后接 catch 来进行捕获异常。
...
模块化开发的最终目的是将程序划分为一个个小的结构，这个结构中可以编写自己的代码，有自己的作用域，不会影响其他结构。

JS 在很长一段时间里都是没有模块化的，但是随着前端项目越来越大，没有模块化变成了 JS 的硬伤。ES2015 将 JS 模块化作为标准（ESM）。在这之前，有很多不同的模块化规范：AMD、CMD、CommonJS等。

#### CommonJS 模块化

CommonJS 模块化规范应用于 NodeJS，按照模块出现顺序==同步==加载模块，每个模块在加载一次之后==会被缓存==（意味着多次加载其实是同一个对象）。导入后导入的对象和暴露的对象是同一个对象。在 Node 中，每一个文件是一个模块，每个模块会被“模块封装器”进行封装，就是在模块外层包裹一个函数。如果想知道当前运行的模块是不是被直接运行的主模块，可以看看 `require.mian === module`。

##### 暴露模块

可以通过在 `module.exports` 上添加属性来暴露模块，也可以通过 `exports` 进行导出：
...
// 但是要注意，exports其实就是对module.exports的引用，最终导出的依旧是module.exports引用的对象，意味着如果直接重写exports对象，那么并不会将这个对象导出。 
```
...
CommonJS 通过 `require` 语句来导入模块，`require` 可以接受一个参数为路径：

```js
let m = require('./module.js')
// 也可以直接解构
let { add, num } = require('./module.js')
...
- 如果传入的是一个 Node 核心模块，比如 path、http，直接返回核心模块，并停止查找。
- 如果传入的是一个路径，会**先将这个路径当做一个文件**来查找，如果有后缀名，就直接找到这个文件；如果没有后缀名，那么会先查找这个对应的没有后缀名的文件，如果没有，会**依次匹配 .js、.json、.node 后缀名**，来查找对应文件。如果**还是没找到，就会将这个路径当做一个文件夹来处理**，依次查找其下的 index.js、index.json、index.node 文件。如果还没有找到，就会报错。
- 如果传入的不是核心模块，也不是路径，会按照当前的路径去查找 `node_modules` 文件夹下有没有这个模块，如果没找到，会依次往上一层目录去查找 `node_modules` 文件夹，然后直到找到根下的 `node_modules`，如果还是没找到，就报错。

##### 内部原理

在导出时，我们向 `module.exports` 挂载了很多的属性，当前文件在 node 中执行的时候，会使用一个函数对我们的整个模块代码进行封装（模块加载器），这个函数传入了 `module.exports` 参数，在函数执行完，这个对象会被修改为真正想要暴露的对象。在被别的模块 `require` 时，对应模块的代码会被执行，结果会被缓存起来，所以多次 require 依旧是同一个模块（在一个模块被加载后，会将该模块的 `module.loaded` 设置为 true，表示已经被加载过了）。
...
- CJS 模块输出的是一个值的复制（虽然也都是引用的同一个对象，但是对象中的基本类型是赋值传递的，不会跟着变），而 ESM 输出的是一个值的引用（即使是基本类型，也是会跟着改的）。（CJS 输出利用的就是 JS 本身的特性，而 ESM 输出的真的是这个变量）
- CJS 是运行时加载，而 ESM 则是编译时输出接口。
...
ES6 模块是在编译时就会进行静态分析，优于模块内其他代码执行，所以我们没有办法通过分支判断语句来导入模块，于是 ES 引入了 import 函数。

使用 import 函数来动态的导入模块。这样引入模块是异步的，返回一个 Promise。
...
- 构建：根据地址查找 js 文件，并且下载，将其解析成模块记录。
- 实例化：对模块记录进行实例化，并且分配内存空间，解析模块的导入导出语句，把模块指向对应的内存地址（静态分析阶段，构建依赖树）。
- 运行：运行代码，计算值，并且将值填充到内存地址中。
...
JSON（JavaScript Object Notation，JS 对象符号）是网络传输的一种重要的数据格式。
...
node package manager，node 包管理工具。我们使用它来管理依赖的包。安装 node 之后就会自动安装完 npm。

可以打开 www.npmjs.com 来查看有哪些发布的包。

#### 配置文件

在每一个项目中，都会有一个配置文件来管理当前项目的依赖。这就是 package.json 文件。

**生成配置文件**

可以在终端中输入命令 `npm init` 来初始化一个包，生成一个 package.json。

一个 package.json 中可能有的字段有：

- name：项目名称
- version：项目版本号
- description：描述信息，项目的基本描述
- author：作者信息
- license：开源协议
- private：私有项目，当设置为 true 时，当前项目是不可以发布的
- main：程序的入口，和 webpack 的入口并不冲突（webpack 会自己找入口），是在发布时会用到的，比如别人把这个包 down 下来之后 require 时，就是通过 main 字段来查找入口文件

其中，name 和 version 是必须填写的字段。

**常见属性**

- scripts：用于配置一些脚本命令，使用键值对来存储，配置完我们可以使用 npm run xxx 来执行。
- dependencies：指定生产依赖，无论开发环境还是生产环境都需要依赖的包。
- DevDependencies：指定开发依赖，这些包只有在开发时依赖，生产是不需要的，比如：webpack。
- peerDependencies：指定对等依赖，也就是依赖某个包，它必须是以另外一个宿主包为前提的；比如安装 element-plus，必须以 Vue3 为前提。
- engines：用于指定 Node 和 NPM 的版本号。
- browserslist：指定配置浏览器的兼容情况，也可以单独写在 .browsersliatrc 文件中。指定之后，可以让 babel 之类的工具来读取。

**依赖的版本号**

依赖的版本号经常类似：^2.0.3 或 ~2.0.3。这是因为 npm 的包遵从 semver 规范，格式为：主版本号.次版本号.修订号。

1. 主版本号：不兼容的版本修改。
2. 次版本号：向下兼容的版本修改。
3. 修订号：向下兼容的修正。

前面的 ^ 指我们在安装的时候主版本号相同的最新版本（但是会先去匹配 package-lock 文件，来下载对应的版本），~ 表示主版本号和次版本号相同，修订号尽量新。
...
`npm install webpack -g` -g 表示全局安装，将这个包安装到全局，并将命令添加到环境变量中。一般来说，工具类的包会使用全局安装。

**局部安装**

项目安装会在当前文件夹下生成一个 node_modules 文件夹，依赖会被安装在这个文件夹下。

`npm install` 会根据 package.json 来对项目的依赖进行安装。

局部安装分为 开发时依赖 和 生产时依赖。

但是其实如果真的把 webpack 安装到了生产依赖中，也不会出错，因为实际打包的时候是根据依赖树确定的。

开发依赖使用 `npm install xxx --save` 或简写为 `npm install xxx -S` 或者直接 `npm install  xxx` 安装。

生产依赖使用 `npm install xxx --save-dev` 或简写为 `npm install xxx -D`。

**install 的原理**

包的开发者可以使用 `npm publish` 对自己开发的包进行发布。使用者使用 `npm i xxx` 对包进行下载安装。

下载下来的是一个压缩包，之后每次下载的时候，会来到缓存来查看有没有对应的缓存，如果有会直接使用本地的压缩包解压。

package-lock.json 文件实际上会记录我们所有依赖的包的具体版本，防止版本不一致导致的 bug。

每次执行 `npm install xxx` 命令时，会去查看有没有 package-lock.json 文件，如果没有，直接构建依赖关系（添加依赖到 package.json），从服务器下载包，解压到 node_modules 中，并生成对应的 package-lock.json 文件。如果有 package-lock.json，会去检查现在要安装的版本和 package-lock.json 中记录的是否一致，不一致的话重新构建依赖关系，去服务器下载包，一致的话去查找本地缓存，从本地或服务器得到包，解压到 node_modules 文件夹。至此，完成安装。

使用 `npm get cache` 命令可以找到缓存的位置。

#### uninstall 命令

全局卸载：`npm uninstall xxx`

局部卸载：`npm uninstall xxx -D` 或 `npm uninstall xxx -S`
...
因为 npm 在早期存在很多缺陷，所以 Facebook、Google 等公司联合推出了一个新的 JS 包管理工具：yarn。
...
因为 npm 服务器部署在外网，我们有可能下载包不是很顺利。我们使用命令 `npm config get registry` 可以查看 npm 的仓库地址，我们会发现是 `https://registry.npmjs.org/`。这里虽然我们可以将 get 换为 set，重新设置国内的镜像地址，但是并不推荐这么做。我们可以安装 cnpm，来设置国内镜像。

```shell
npm i cnpm -g --registry=https://registry.nlark.com/
# 这时再看cnpm的仓库地址
...
npx 是不需要再次安装的，是在 npm 5.2 后自带的一个命令。npx 的作用非常多，但比较常见的是使用它来调用项目中某个模块的指令。

比如，我们全局和项目都安装了某个包，比如 webpack，那么如果我们直接执行 webpack 命令，使用的是全局的 webpack。但是我们希望使用当前项目的 webpack 来打包，那么我们可以使用 npx webpack 来调用当前项目中的webapck。我们也可以把它配置在 scripts 里，也可以找到本地的 webpack。
...
JSON Web Token，通过数字签名的方式，以 JSON 对象为载体，在不同的服务器终端之间安全的传输信息。

JWT 最常见的场景就是授权认证，一旦用户登录，后续每个请求都将会包含 JWT，系统在每次处理用户请求之前，都要先进行 JWT 校验，通过之后再进行处理。

JWT 由三部分组成，由点（.）分隔。分别是：

- header，保存了使用了什么加密算法，使用 base64 编码。
- payload，载荷，保存了信息，使用 base64 编码。
- signature，签名，将前两部分的 base64 编码使用点拼接起来，然后所得内容通过规定的加密算法加盐加密。

最后这三部分一起组成了 JWT，这样一来，信息没有存储在服务器端，降低了服务器内存压力。
...
在需要对 DOM 元素进行底层操作时，我们可以用到自定义指令。
...
向插槽提供内容时，可以在一个`<template>`上使用`v-slot`指令，并以参数的形式提供名称。
...
有时组件与组件之间会存在相同代码逻辑，我们希望对相同的代码逻辑进行抽取。

在 Vue2 和 Vue3 中都支持的一种方式是使用 Mixin 来完成。

Mixin 提供了一种非常灵活的方式，来分发 Vue 组件中的可复用功能。一个 Mixin 对象可以包含任何组件选项。当组件使用 Mixin 对象时，所有 Mixin 对象的选项将被混合进入组件本身的选项中。
...
如果命名发生了冲突，在 data 函数的返回对象中默认会进行合并，产生冲突时，保留组件中的数据。生命周期不会冲突，全部都会执行。

也可以使用全局混入，对所有组件进行混入。

通过调用 createApp 生成的 app 身上的 mixin 方法进行混入：
...
Vue 在模板解析时，使用 h 函数创建了虚拟 DOM。h 函数可以接受一个数组，来嵌套的调用 h 函数，以创建一个树结构：

```js
h('ul', {class: {'list': true}}, [
  h('li', {}, 'listItem1'),
  h('li', {}, 'listItem2'),
  h('li', {}, 'listItem3')
])
// 创建的虚拟DOM树为：
{
  ...
  "data": {},
  "children": [
    {"sel": "li", "text": "listItem1"},
    {"sel": "li", "text": "listItem2"},
    {"sel": "li", "text": "listItem3"}
  ]
};
```
...
在 Vue 中，有一个 VNode 类，使用它可以实例化不同类型的 vnode 实例，来表示不同类型的 DOM 元素。通过 vnode，可以创建一个真实 DOM 元素，再插入到视图中。

由于每次都是先创建 vnode，再创建 DOM，所以可以将 vnode 储存起来，与下一次进行对比。来找到哪里不同，再去修改 DOM。所以说**使用 vnode 来缓存状态是它最重要的作用**。
...
6. 函数式组件
   ...
   和组件节点类似，但有两个独有属性：functionalContext 和 functionalOptions。

#### patch

patch 是虚拟 DOM 最核心的部分，负责将 vnode 渲染成真实的 DOM。

patch 并不是暴力的渲染整个负责的 DOM，而是会将新旧 vnode 进行对比，之后再去对需要的节点进行更新。本质上是用 JS 的运算成本替换 DOM 操作的成本（最小量更新）。

在对两个虚拟节点进行 比对（diff）时，只有**两个节点是同一个虚拟节点时，才会进行精细化比较**（选择器相同，key 相同），否则会直接删除旧节点，插入新节点。

而且**只会进行同层次节点的比较**，跨层次是不会进行比较的，也是直接删除旧的，插入新的。

patch 方法被调用后，首先判断传入的 oldVNode 是不是虚拟节点，如果是真实 DOM，说明需要将新的 VNode 渲染到这个 DOM 元素上，所以会将 oldVNode 转化为一个空的 VNode，这样后面的逻辑就统一了。

如果是同一个虚拟节点，会对这两个节点进行精细化比对，如果不是同一个节点，那么会直接删除旧的，插入新的。
